...
| `checkout()` | UUID + gönderilecek gövdeyi bir denemeye bağlar. Tek POST devam ederken buton disabled. Başarı sepet/adres temizlenir, orderID/tutar özeti, katalog güncellemesi. Yalnız tanınan400 doğrulama/stok/ürün/tutar retlerinde düzeltmeye izin verir. Ağ/5xx ve403/429 gibi ön koşul retlerinde pendingOrder korunur; aynı gövde/key ile tekrar düğmesi gelir. Sonraki isteğin reddi, önceki denemenin kaydedilmediği anlamına gelmez.401 ise api üzerinden oturumu temizler. |
...
| `tests/frontend.test.js` | Gerçek public/app.js kaynağını Node vm bağlamında çalıştırır. `element()` küçük DOM taklidini, `frontend(checkoutReplies)` HTML kimlikleriyle elemanları, kontrollü fetch yanıtlarını ve boot/addToCart hazırlığını kurar; dönen run bağlamda checkout çağırır. `success()` tekrar yanıtı örneğidir. Beş parametrik4xx senaryosu ile yanıt kaybından sonra anahtar/gövde ve düzenleme kilidinin korunması; CSRF otomatik tekrar zinciri; kesin validasyon retlerinde yeni deneme; stok yenileme olmak üzere8 test vardır. Yeni bağımlılık veya gerçek ağ gerekmez; DOM zamanlaması, yerleşim ve odak doğrulaması değildir. |
...
CI iki ayrı işte aynı yerel komutları çalıştıracak şekilde yazıldı. İlk uygulama tesliminde commit/push yapılmamıştı; sonraki açık kullanıcı talimatı commit ve push kapsamını ekledi. Yerel başarı, uzak Actions sonucunun yerine geçmez. Canlı AWS, gerçek WhatsApp ve fiziksel telefon/ekran okuyucu cihaz testi yoktur. Temel klavye/semantik/mobil viewport kontrolleri, tam erişilebilirlik sertifikası değildir.
